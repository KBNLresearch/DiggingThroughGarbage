...
### if a string is more than 18 characters in length, it is garbage
def wordlength(word):
    if len(word) > 18:
        return 1
    else:
...
### if a word contains more than one punctuation character it is garbage
...
### If there are three or more identical characters in a row, it is garbage
...
def Main(word, consonants_list, vowels_list, dutch_char_list, accents_list):
    ...
    garbage_count = sum([length, punct, identical, con_vow_ratio,  consecutive, dutch])
    if garbage_count > 0:
        return 1
    else: 
        return 0